...
## 16. 新手任务引导增量约束（N 阶段；N-001 已完成，N-002～N-004 待分配）

N-001～N-004 以 development-plan §9 为任务源，不改变 INV-001～020。新增入口是请求草稿辅助，不是第二批准通道，不扩大 read/write/edit、恢复或清理范围。当前公开 0.8.0 不因此获得新能力；N-001 的实际验收记录见 references §45，N-002～N-004 仍待执行。

| 任务 | 必需增量断言 | 当前证据 |
|---|---|---|
| N-001 | INV-003/005/008/012/015/016/019/020：输入与投影限额、无原始/秘密输出、非空编辑器保留、异步后同步检查填入、会话失效、无自动发送/批准、真实 Pi InteractiveMode 合约 | PASS（references §45；锁定包本地实现与测试，真实人工 TUI 仍 NOT_RUN，待用户评审） |
| N-002 | INV-002/003/005/007～010/012/013/016/019/020：结构事实生成建议、未知/多原因不降级、不重放原批次、可能已执行不重试、目标身份与生命周期、无旧结果覆盖 | NOT_RUN |
