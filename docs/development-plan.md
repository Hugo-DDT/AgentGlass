...
> 版本：3.3 · 2026-09-15 · 正式任务源 · 当前发布基线仍为 `@hugo-ddt/agentglass@0.8.0`；N-001/N-002 用户评审完成，N-003～N-004 未分配；Pi Web W-001～W-005 继续暂缓

## 当前状态与下一步

**2026-09-15 当前阶段修订（v3.3）：** 在恢复 `0.8.0`、暂缓 Pi Web 的决定上，用户选定 A＋C＋D：常用任务起步器、卡住后的处理入口、结果后的下一步菜单。N-001/N-002 用户评审已完成；N-003～N-004 不自动开始。W-001～W-005 的历史记录不作为当前能力或下一任务授权，重启仍须新分配与新证据。
...
R-001～R-004 是已完成首发的历史任务；N-001/N-002 用户评审已完成，下一可分配任务为 N-003（不自动开始），当前不进入 Pi Web。Pi 超限预览等外部限制继续按 `0.8.0` 已知限制处理；详见第 6 节与 references §40。各轮可复制[新手阶段 Codex Prompt 手册](novice-codex-prompts.md)，完成一轮并评审后再明确分配下一轮。
...
## 9. 新手文件任务引导（N 阶段；N-001/N-002 已完成，N-003～N-004 待分配）
...
| 轮次 / Task | 依赖 | 交付 | 当前状态 |
|---|---|---|---|
| 第一轮 N-001 常用任务起步器 | 本阶段规格；核对实际 0.8.0 基线与锁定 Pi | 三类引导、统一草稿填入、输入框保护与宿主合约测试 | COMPLETED（2026-09-15，本地，真实 InteractiveMode 合约已补测，用户评审完成） |
| 第二轮 N-002 卡住后的处理入口 | N-001 必需门通过且完成评审 | 最近问题结构事实、适用处理菜单与不重试边界 | IN_PROGRESS（2026-09-16，本地修复候选已通过，等待远端 Windows CI 复测） |
