...
> 版本：3.2 · 2026-09-15 · 正式任务源 · 首发 `@hugo-ddt/agentglass@0.8.0` 保持为当前发布基线；Pi Web W-001～W-005 暂缓，不属于当前发布线；此前 Pi Web 联调与制品记录保留为历史证据，不代表当前版本能力或公开兼容承诺

## 当前状态与下一步

**2026-09-15 当前阶段修订（v3.2）：** 用户决定暂缓 Pi Web 插件适配，当前仓库与产品发布线恢复到已发布的 `@hugo-ddt/agentglass@0.8.0`。第 8 节 W-001～W-005 保留为未来计划但全部暂缓；此前 W-001～W-005 的本地联调、制品和失败门记录只用于历史审计，不作为当前版本能力、公开宿主兼容或下一任务授权。恢复适配必须重新明确分配，并重新固定候选、制品和安全证据。

R-001 已按明确分配执行至当前证据边界。历史 Release 记录中的候选 HEAD 不代表本轮工作树；本轮实际 AgentGlass HEAD 为 `814827da5df45d4b48dbadc24ddcc7913fa105b7`，被忽略的文档已直接读取。历史验收是依赖证据，不是本次重跑结果。
...
| 性能与资源 | 当前发布线沿用 `0.8.0` 的 R-003/R-004 证据；W-005 的 `0.8.1` 测量仅作历史记录 | 暂不执行 Pi Web 或新候选性能门 |
| 发布 | `RELEASE_READY=PASS` 仅对应已发布的 `0.8.0`；Pi Web 阶段不发布 | 暂缓，无下一发布候选 |

R-001～R-004 是已完成首发的历史任务；当前不进入 Pi Web 阶段，也不自动分配下一任务。Pi 超限预览等外部限制继续按 `0.8.0` 已知限制处理；详见第 6 节与 references §40。

分配任务时可复制 [Release Codex Prompt 手册](release-codex-prompts.md) 的对应完整代码块；该手册只展开本计划的执行要求，不构成新的任务规范或当前执行授权。
